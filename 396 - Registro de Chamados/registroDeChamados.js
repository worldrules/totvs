...
//alterando
var loadUsersBySelectedRole = function(){
	var grupoSelecionado = $('#envioPara').val();
	...
	//                                        3ª Coluna
	var c1 = DatasetFactory.createConstraint("workflowColleagueRolePK.roleId", grupoSelecionado, grupoSelecionado, ConstraintType.MUST);
	var dsColleagueGroup = DatasetFactory.getDataset("workflowColleagueRole", null, new Array(c1), null);
	for(var i = 0; i < dsColleagueGroup.values.length; i++){
		usersByRole.push(dsColleagueGroup.values[i]['workflowColleagueRolePK.roleId'])
	}
	var dsColleague = DatasetFactory.getDataset("colleague", null, null, null);
	
	for(var i = 0; i < dsColleague.values.length; i++){
		if($.inArray(dsColleague.values[i]['workflowColleagueRolePK.colleagueId'], usersByRole) !== -1){
			$('#responsavel').append('<option class="reponsavelOption" value="'+dsColleague.values[i]['workflowColleagueRolePK.colleagueId']+'">'+dsColleague.values[i]['colleagueName']+'</option>')
			$('#responsavelFormArea').append('<option class="responsavelFormAreaOption" value="'+dsColleague.values[i]['workflowColleagueRolePK.colleagueId']+'">'+dsColleague.values[i]['colleagueName']+'</option>')
		}	
	}
}
